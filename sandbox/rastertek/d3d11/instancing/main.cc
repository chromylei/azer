...
#define EFFECT_GEN_DIR "out/dbg/gen/sandbox/rastertek/d3d11/instancing/"
#define SHADER_NAME "instancing"
using base::FilePath;
...
class MainDelegate : public azer::WindowHost::Delegate {
 public:
  ...
  virtual void OnRenderScene(double time, float delta_time) {
    ...
    for (int i = 0; i < std::min(10u, arraysize(kPosition)); ++i) {
      azer::Matrix4 world = std::move(azer::Translate(kPosition[i]));
      azer::Matrix4 wvp = projection_ * camera_.GenViewMat() * world;
      effect_->set_vs_wvp(i, wvp);
      effect_->set_vs_world(i, world);
    }

    effect_->Use(rs);
    rs->RenderInstanced(arraysize(kPosition), vb_.get(), azer::kTriangleList);
 ...
 private:
  ...
  std::unique_ptr<InstancingEffect> effect_;
