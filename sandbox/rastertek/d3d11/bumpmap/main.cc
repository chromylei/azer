...
#define EFFECT_GEN_DIR "out/dbg/gen/sandbox/rastertek/d3d11/bumpmap/"
#define SHADER_NAME "bumpmap"
using base::FilePath;
...
class MainDelegate : public azer::WindowHost::Delegate {
 public:
  ...
  void Init() {
    ...
    std::vector<BumpMapEffect::Vertex> vertices;
    ...
    tex_bumpmap_.reset(rs->CreateTextureFromFile(
        azer::Texture::k2D,
        base::FilePath(TEXT("sandbox/rastertek/media/bump01.dds"))));
    ...
    light_.set_directional(azer::Vector4(1.0f, 0.0f, -1.0f, 0.0f));
  ...
  virtual void OnRenderScene(double time, float delta_time) {
    ...
    rs->SetCullingMode(azer::kCullNone);
    effect_->set_ps_DiffuseTex(tex_diffuse_.get());
    effect_->set_ps_BumpMapTex(tex_bumpmap_.get());
 ...
 private:
  ...
  std::unique_ptr<BumpMapEffect> effect_;
