...
#define EFFECT_GEN_DIR "out/dbg/gen/sandbox/rastertek/d3d11/alphamap/"
#define SHADER_NAME "alphamap"
using base::FilePath;
...
class MainDelegate : public azer::WindowHost::Delegate {
 public:
  ...
  void Init() {
    std::string shader_name = "alphamap";
    ...
    tex3_.reset(rs->CreateTextureFromFile(
        azer::Texture::k2D,
        base::FilePath(TEXT("sandbox/rastertek/media/alpha01.dds"))));
  ...
  virtual void OnRenderScene(double time, float delta_time) {
    ...
    rs->SetCullingMode(azer::kCullNone);
    effect_->set_ps_StoneTex(tex1_.get());
    effect_->set_ps_DirtTex(tex2_.get());
    effect_->set_ps_AlphaTex(tex3_.get());
 ...
 private:
  ...
  std::unique_ptr<AlphaMapEffect> effect_;
