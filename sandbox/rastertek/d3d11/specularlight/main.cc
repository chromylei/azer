...
#define EFFECT_GEN_DIR "out/dbg/gen/sandbox/rastertek/d3d11/specularlight/"
#define SHADER_NAME "specular"
using base::FilePath;
...
class MainDelegate : public azer::WindowHost::Delegate {
 public:
  ...
  void Init() {
    ...
    effect_.reset(new SpecularEffect(shaders.GetShaderVec(), rs));
    ...
    memcpy(data_->pointer(), (uint8*)&vertices[0],
           sizeof(SpecularEffect::Vertex) * vertices.size());
  ...
  virtual void OnRenderScene(double time, float delta_time) {
    ...
    effect_->set_ps_diffuse(azer::vec4(1.0f, 0.0f, 1.0f, 1.0f));
    effect_->set_vs_camera_pos(azer::vec4(camera_.position(), 1.0f));
    effect_->set_ps_specular_power(20.0f);
    effect_->Use(rs);
