...
#include "afx_projective_tex_effect.h"
#include <tchar.h>

#define EFFECT_GEN_DIR "out/dbg/gen/sandbox/rastertek/d3d11/projectivetex/"
#define SHADER_NAME "afx_projective_tex_effect"
...
class MainDelegate : public azer::WindowHost::Delegate {
 public:
  ...
  virtual void OnQuit() {}
 private:
  std::unique_ptr<ProjectiveTexEffect> effect_;
...
void MainDelegate::Init() {
  ...
  effect_.reset(new ProjectiveTexEffect(shaders.GetShaderVec(), rs));

  {
    ...
    memcpy(plane_data_->pointer(), (uint8*)&vertices[0],
           sizeof(ProjectiveTexEffect::Vertex) * vertices.size());
  ...
  projected_tex_.reset(rs->CreateTextureFromFile(
      azer::Texture::k2D,
      base::FilePath(TEXT("sandbox/rastertek/media/dx11.dds"))));
...
void MainDelegate::OnRenderScene(double time, float delta_time) {
  ...
  effect_->set_ps_ProjTex(projected_tex_);
