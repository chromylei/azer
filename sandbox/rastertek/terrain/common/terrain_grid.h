...
#include "sandbox/rastertek/terrain/common/heightmap.h"

class TerrainGridEffect : public azer::Effect {
 public:
  TerrainGridEffect(azer::RenderSystem* rs)
      : azer::Effect(rs) {
    Init(rs);
  }
...
class TerrainGrid {
 public:
  TerrainGrid(azer::RenderSystem* rs, azer::TechniquePtr ptr) {
    effect_.reset(new TerrainGridEffect(rs));
    effect_->SetTechnique(ptr);
  }

  TerrainGridEffect* effect() { return effect_.get();}
 ...
 private:
  ...
  std::unique_ptr<TerrainGridEffect> effect_;
...
typedef std::shared_ptr<TerrainGrid> TerrainGridPtr;
