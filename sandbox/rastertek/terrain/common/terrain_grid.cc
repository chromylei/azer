...
void TerrainGridEffect::Init(azer::RenderSystem* rs) {
  GpuConstantsTable::Desc vs_table_desc[] = {
    GpuConstantsTable::Desc("vp", azer::GpuConstantsType::kMatrix4,
                            offsetof(TerrainGridEffect::vs_cbuffer, vp), 1),
    GpuConstantsTable::Desc("vp", azer::GpuConstantsType::kMatrix4,
                            offsetof(TerrainGridEffect::vs_cbuffer, world), 1), 
  };
  ...
  GpuConstantsTable::Desc ps_table_desc[] = {
    GpuConstantsTable::Desc("diffuse", azer::GpuConstantsType::kVector4,
                            offsetof(TerrainGridEffect::ps_cbuffer, diffuse), 1), 
  };
...
void TerrainGrid::Init(const base::FilePath& filepath) {
...
void TerrainGrid::Render(azer::Renderer* rs) {
  const int kRow = height_map_.width;
  const int kColumn = height_map_.height;
  const int kTriangleNum = (kRow - 1) * (kColumn - 1) * 2;
