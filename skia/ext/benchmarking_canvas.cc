...
public:
  ...
  virtual void clipRect(const SkRect& rect, SkRegion::Op op,
                        bool doAa) OVERRIDE {
    AutoStamper stamper(this);
    return SkProxyCanvas::clipRect(rect, op, doAa);
  }

  virtual void clipRRect(const SkRRect& rrect, SkRegion::Op op,
                         bool doAa) OVERRIDE {
    AutoStamper stamper(this);
    return SkProxyCanvas::clipRRect(rrect, op, doAa);
  }

  virtual void clipPath(const SkPath& path, SkRegion::Op op,
                        bool doAa) OVERRIDE {
    AutoStamper stamper(this);
    return SkProxyCanvas::clipPath(path, op, doAa);
  }

  virtual void clipRegion(const SkRegion& region,
